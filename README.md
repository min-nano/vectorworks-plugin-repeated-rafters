...
パスプラグインオブジェクトの**パスが屋根の水平投影面**（閉ポリゴン）になります。**地廻り基準線**は、2 つの**コントロールポイント**（`BaseStart`・`BaseEnd`）で指定する**内蔵の直線**です。垂木はこの基準線に**直交する向き**で**指定間隔**に並び、各垂木は基準線から**棟側（高い側）と軒先側（低い側）の両方向**へ、屋根の水平投影面でクリップした長さで伸び、**勾配なりに傾けて**描かれます（立ち上がり＝水平投影長 × 勾配 ÷ 10）。垂木は 1 本ずつ構造材ツール（`StructuralMember`）の傾斜材として描かれます。
...
> `BaseStart`・`BaseEnd` はコントロールポイント型のパラメータです。VectorWorks はコントロールポイントの座標を、パラメータ名末尾に `X`／`Y` を付けたフィールド（`BaseStartX`・`BaseStartY`・`BaseEndX`・`BaseEndY`）で保持します（コード側はこの名前で読み取ります）。新規オブジェクトで基準線が見える位置に出るよう、プラグイン定義でコントロールポイントの既定座標を屋根の想定位置に設定しておくとよいです。

## 使い方

1. `垂木群` ツールで屋根の水平投影面をパス（閉じた多角形）として描きます。
2. 2 つのコントロールポイント（`BaseStart`・`BaseEnd`）をドラッグして、地廻り基準線を配置します（軒の出があるぶん、屋根投影面の軒先より内側に置きます）。
