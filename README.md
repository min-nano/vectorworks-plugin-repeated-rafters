...
- **パラメータ**:
  ...
  | `2DDisplay` | 文字 | 2D 表現（`2DDisplay` へプロキシ）。実線／中心線／幅／中心線と幅／面なし ＝ `solid`／`center`／`width`／`widthcenter`／`none` | `width`（幅） |

> **垂木のクラスは PIO オブジェクト自身のクラス**を使います（`vs.GetClass` で読み取り、各垂木を同じクラスに置きます）。専用のクラスパラメータは持たせません。垂木を分けたいクラスに `垂木群` オブジェクトを置いてください（PIO のクラスが未設定の場合は `一般` にフォールバックします）。
...
> **軸組ツール（`FramingMember`）パラメータのプロキシについて**: 上表の `config` 以降は、**軸組ツールで設定するパラメータを PIO パラメータとして公開**し、垂木の描画時に各垂木（`FramingMember`）の**同名レコードフィールドへそのまま転送（プロキシ）**するものです。垂木の要点のみを対象とし、断面形状の各種オプション等は VectorWorks の既定に委ねます。PIO 側のパラメータ（フィールド）名は転送先の `FramingMember` フィールド名と同一にして対応を明確にしています。`2DDisplay`（2D 表現）は `FramingMember` 側では「実線／中心線／幅／中心線と幅／面なし」のポップアップですが、プロキシで転送するのは内部値（`solid`／`center`／`width`／`widthcenter`／`none`）です。PIO のパラメータは、これらの内部値を選択肢にしたポップアップとして登録すると OIP から選べて便利です（既定は `width` ＝幅）。
...
3. オブジェクト情報パレット（OIP）で勾配・垂木断面・間隔・2D 表現などを設定します（垂木のクラスは `垂木群` オブジェクト自身のクラスになります）。
